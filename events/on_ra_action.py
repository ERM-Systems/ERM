...
class OnRAAction(commands.Cog):
    def __init__(self, bot: commands.Bot):
        self.bot = bot

    @commands.Cog.listener()
    async def on_ra_accept(self, s_loa: dict, role_ids: list[int], accepted_by: int):
        ...
        ra_channel_id = settings.get("reduced_activity", {}).get("channel", 0)
    ...
    async def on_ra_deny(
        ...
        try:
            ...
            denied_by_user = self.bot.get_user(denied_by)
...
async def setup(bot):
    await bot.add_cog(OnRAAction(bot))
